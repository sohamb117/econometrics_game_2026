...
#delimit;
capture log close;
log using "C:\Incentives_Matter_Data\Incentives_FINAL\Log Files\Table8MidTest.log", replace;
...
regress entry treat, cluster(schid);
regress missing treat, cluster(schid);
regress missing treat if stayer==1 | missing==1, cluster(schid);
...
**PANEL A/COL 3;
regress missing treat, cluster(schid);
outreg treat using "C:\Incentives_Matter_Data\Incentives_FINAL\Output\TABLE8PANELACOL3.csv", bdec(2) se 3aster rdec(2) comma bracket replace;
...
outreg treat_missing using "C:\Incentives_Matter_Data\Incentives_FINAL\Output\TABLE8PANELACOL3.csv", bdec(2) se 3aster rdec(2) comma bracket append;
...
**PANEL B/COL 1 AND COL 2;
sort treat;
by treat: sum mid_writ z_add_mid_math z_add_mid_lang z_add_mid_total;

**PANEL B/THIRD COLUMN;
regress mid_writ treat, cluster(schid);
outreg treat using "C:\Incentives_Matter_Data\Incentives_FINAL\Output\TABLE8PANELBCOL3.csv", bdec(2) se 3aster rdec(2) comma bracket replace;

regress z_add_mid_math treat, cluster(schid);
outreg treat using "C:\Incentives_Matter_Data\Incentives_FINAL\Output\TABLE8PANELBCOL3.csv", bdec(2) se 3aster rdec(2) comma bracket append;
